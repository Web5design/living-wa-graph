app.factory('leafletMarkersHelpers', function ($rootScope, leafletHelpers, $log) {
    ...
    return {
        ...
        createCircleMarker: function(markerData) {
            if (!isDefined(markerData)) {
                $log.error('[AngularJS - Leaflet] The marker definition is not valid.');
                return;
            }
            ...
            var markerOptions = {
                stroke: isDefined(markerData.stroke) ? markerData.stroke : true,
                color: isDefined(markerData.color) ? markerData.color : '#03f',
                weight: isDefined(markerData.weight) ? markerData.weight : 5,
                opacity: isDefined(markerData.opacity) ? markerData.opacity : 0.5,
                fill: isDefined(markerData.fill) ? markerData.fill : true,
                fillColor: isDefined(markerData.fillColor) ? markerData.fillColor : '#03f',
                fillOpacity: isDefined(markerData.fillOpacity) ? markerData.fillOpacity : 0.2,
                dashArray: isDefined(markerData.dashArray) ? markerData.dashArray : null,
                lineCap: isDefined(markerData.lineCap) ? markerData.lineCap : null,
                lineJoin: isDefined(markerData.lineJoin) ? markerData.lineJoin : null,
                clickable: isDefined(markerData.clickable) ? markerData.clickable : true,
                pointerEvents: isDefined(markerData.pointerEvents) ? markerData.pointerEvents : null,
                className: isDefined(markerData.className) ? markerData.className : '',
                message: isDefined(markerData.message) ? markerData.message : ''
            };

            var radius = isDefined(markerData.radius) ? markerData.radius : 10

            return new L.circleMarker(markerData, markerOptions).setRadius(radius);
        },
